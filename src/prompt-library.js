...
import { EnhancementAgentImpl } from './services/enhancement-agent';
...
export class PromptLibrary {
    ...
    async initializeStorage() {
        ...
        this.integratedStorage = new FileSystemIntegratedStorage(this.config.storageDir, this.config.enableFileWatcher);
     ...
     */
    async initializeVariableManager() {
     ...
     */
    async initializeEnhancementAgent() {
    ...
    async initializePromptManager() {
        ...
        if ('initialize' in this.promptManager && typeof this.promptManager.initialize === 'function') {
    ...
    async getStatus() {
        ...
        if (this.isInitialized && this.promptManager) {
            try {
                if ('getServiceStats' in this.promptManager && typeof this.promptManager.getServiceStats === 'function') {
                    status.storageStats = await this.promptManager.getServiceStats();
                }
            }
            catch (error) {
    ...
    async checkProviderHealth() {
        ...
        const healthChecks = providers.map(async (provider) => {
            try {
                ...
                const hasRequiredMethods = typeof adapter.render === 'function' &&
    ...
    async validateInitialization() {
        ...
        if (!this.integratedStorage)
            errors.push('Storage not initialized');
        if (!this.llmService)
            errors.push('LLM service not initialized');
        if (!this.providerRegistry)
            errors.push('Provider registry not initialized');
        if (!this.versionManager)
            errors.push('Version manager not initialized');
        if (!this.ratingSystem)
            errors.push('Rating system not initialized');
        if (!this.variableManager)
            errors.push('Variable manager not initialized');
        if (!this.enhancementAgent)
            errors.push('Enhancement agent not initialized');
        if (!this.promptManager)
            errors.push('Prompt manager not initialized');
        if (errors.length > 0) {
