...
export class EnhancementAgentImpl implements EnhancementAgent {
  ...
  async analyzePrompt(prompt: HumanPrompt | StructuredPrompt): Promise<{
    suggestions: string[];
    quality_score: number;
    missing_elements: string[];
  }> {
    ...
    if (isHuman) {
      const humanPrompt = prompt as HumanPrompt;
...
Focus on clarity, specificity, and maintainability. Extract variables for any content that should be dynamic or reusable.`;
  ...
  private calculateConfidence(
    ...
    if (enhanced.system.length === 0) confidence -= 0.2;
    if (!enhanced.user_template) confidence -= 0.3;
    if (enhanced.rules.length === 0) confidence -= 0.1;
