...
export class PromptVersionManager implements VersionManager {
  /**
   * Create a new version of a prompt
   */
  createVersion(prompt: PromptRecord, options: VersionCreateOptions): PromptVersion {
   ...
   */
  private compareMetadata(oldPrompt: PromptRecord, newPrompt: PromptRecord, changes: VersionDiff[]): void {
   ...
   */
  private compareHumanPrompt(oldPrompt: PromptRecord, newPrompt: PromptRecord, changes: VersionDiff[]): void {
   ...
   */
  private compareStructuredPrompt(oldPrompt: PromptRecord, newPrompt: PromptRecord, changes: VersionDiff[]): void {
   ...
   */
  private compareVariables(oldPrompt: PromptRecord, newPrompt: PromptRecord, changes: VersionDiff[]): void {
   ...
   */
  private rulesEqual(a: Array<{name: string; description: string}>, b: Array<{name: string; description: string}>): boolean {
    if (a.length !== b.length) return false;
    return a.every((rule, index) => 
      rule.name === b[index].name && rule.description === b[index].description
    );
  }
   ...
   */
  private variablesEqual(a: any, b: any): boolean {
    return (
      a.key === b.key &&
      a.label === b.label &&
      a.type === b.type &&
   ...
   */
  private groupChangesByType(changes: VersionDiff[]): {
    added: VersionDiff[];
    modified: VersionDiff[];
    removed: VersionDiff[];
  } {
    return {
