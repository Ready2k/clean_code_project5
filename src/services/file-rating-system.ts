...
export class FileRatingSystem implements RatingSystem {
   ...
   * Rate a prompt
   */
  async ratePrompt(promptId: string, userId: string, rating: Omit<Rating, 'id' | 'created_at'>): Promise<void> {
   ...
   */
  async getAverageRating(promptId: string): Promise<number> {
   ...
   */
  async logRun(runLog: Omit<RunLog, 'id' | 'created_at'>): Promise<void> {
   ...
   */
  async getPromptRatingAggregationAllVersions(promptId: string): Promise<RatingAggregation> {
    const ratings = await this.getPromptRatings(promptId);
   ...
   */
  async getPromptVersionRatingAggregation(promptId: string, version: number): Promise<RatingAggregation> {
   ...
   */
  async getRunStatistics(promptId: string): Promise<{
    total_runs: number;
    success_rate: number;
